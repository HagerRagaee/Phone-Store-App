...
//   static Future<List<SaleRecord>> getSalesByOneDate(DateTime? date) async {
//     List<SaleRecord> allSales = [];
//     try {
//       DateTime startOfDay = DateTime(date!.year, date.month, date.day);
//       DateTime endOfDay =
...
//   static Future<void> updateSaleRecord(
//       SaleRecord oldSale, SaleRecord updatedSale, BuildContext context) async {
//     // If the item name changed
//     var newQuantity = oldSale.quantitySold;

//     if (oldSale.itemName != updatedSale.itemName) {
//       await returnSaleRecord(oldSale.docId!, context); // Remove old sale record
//       await saveSaleRecord(updatedSale, context); // Save updated sale record
//       return; // Exit since we have already handled the update
//     }
